...
- **Multi-Step (Sequential):** If the model calls a tool, receives a result, and calls _another_ tool (within the same turn), **both** function calls have signatures. You must return **all** accumulated signatures in the history.
